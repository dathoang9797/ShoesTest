...
class Shoes {
    ...
    getCartItemIndex(cartItem) {
        ...
        const cartItemIndex = shoesList.findIndex((shoes) => shoes.id === cartItem.id);
    ...
    getPriceCartItem = (that) => {
        return +$(that).parents('.cart__item__right__actions').siblings('.cart__item__right__price').text().split('$').reverse()[0];
    };
    getAmountCartItem = (that) => {
        return +$(that).parents('.cart__item__right__actions').find('.cart__item__right__count__number').text();
    };
    getCartItemCount = (that) => {
        return $(that).siblings('.cart__item__right__count__number');
    };
    getCardItemButton = (cartItemId) => {
        const cardItemButton = $(`.card__item#${+cartItemId}`).find('.card__item__button');
    ...
    async showShoesCardItem(cardItems) {
        ...
        if (localStorage.getItem('shoes')) {
            ...
            shoesListInLocalStorage.forEach((shoesItem) => {
                const cardItemButton = $(`.card__item#${+shoesItem.id}`).find('.card__item__button');
                cardItemButton.addClass('inactive').get(0).innerHTML = cardItemButtonIcon;
            });
        }
    }
    ...
    async addCartItem(cartItems, cartTitleRight, event) {
        ...
        cardItemButton.addClass('inactive animate__animated animate__fadeInDown').get(0).innerHTML = cardItemButtonIconContent;
        cartItem.count = 1;
    ...
    async removeCartItem(cartTitleRight, event) {
        ...
        cardItemButton.removeClass('inactive animate__animated animate__fadeInDown');
        cardItemButton.get(0).innerHTML = "<p class='animate__animated animate__zoomIn'>ADD TO CART</p>";
        cartTitleRight.text(`$${(cartTitleRightPrice - priceCartItem * amountCartItem).toFixed(2)}`);
    ...
    async changeAmountCartItem(cartItems, cartTitleRight, event) {
        ...
        const shoesList = this.getAllShoesFromLocalStorage();
        const cartItemIndex = this.getCartItemIndex(cartItem);
        if (cartItemButtonContent === '-' && cartTitleRight.text(`$${(cartTitleRightPrice - priceCartItem).toFixed(2)}`) && cartItemCountNumber.text(`${+cartItemCountNumber.text() - 1}`)) {
            if (+cartItemCountNumber.text() === 0) {
                $(event.currentTarget).css('pointer-events', 'none');
                cardItemButton.removeClass('inactive animate__animated animate__fadeInDown').get(0).innerHTML = "<p class='animate__animated animate__zoomIn'>ADD TO CART</p>";
                shoesList.splice(cartItemIndex, 1);
                this.cartItemRemoveFromChild(event.currentTarget);
                this.checkCartTextShowOrHide(cartItems);
                this.setShoesInLocalStorage(shoesList);
