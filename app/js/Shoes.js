class Shoes {
  ...
  async showShoesCardItem(cardItems) {
    const shoesList = await this.getAllShoes();
    const cardItemButtonIcon = `<div class="card__item__button__cover__check__icon">
    ...
    shoesList.forEach((shoes, index) => {
      ...
      <div class="card__item" id="${id}">
        ...
        <div class="card__item__name">${name}</div>
        <div class="card__item__description"> ${description} </div>
        <div class="card__item__bottom ">
          <div class="card__item__price">$${price}</div>
          <div class="card__item__button">
  ...
  showShoesCartItem(cartItems, cartTitleRight) {
    ...
    shoesList.forEach((shoes, index) => {
      ...
      sum += price * count;
      const template = `
          <div class="cart__item animate__animated animate__fadeInRight" id="${id}">
        <div class="cart__item__left">
          <div
            class="cart__item__left__image"
            style="background-color: ${color}"
          >
            <div class="cart__item__image__block">
              <img src="${image}" />
            </div>
          </div>
        </div>
        <div class="cart__item__right ">
          <div class="cart__item__right__name">${name}</div>
          <div class="cart__item__right__price">$${price}</div>
          <div class="cart__item__right__actions">
            <div class="cart__item__right__count">
              <div class="cart__item__right__count__button">-</div>
              <div class="cart__item__right__count__number">${count}</div>
              <div class="cart__item__right__count__button">+</div>
            </div>
            <div class="cart__item__right__remove">
              <img
  ...
  async addCartItem(cartItems, cartTitleRight, event) {
    ...
    const cardItemButtonIconContent = `
    <div class="card__item__button__cover__check__icon">
      <img src="assets/check.png" alt="" />
    </div>`;
    ...
    const template = `
      ...
      <div class="cart__item__right">
        ...
        <div class="cart__item__right__actions">
          <div class="cart__item__right__count">
            <div class="cart__item__right__count__button">-</div>
            <div class="cart__item__right__count__number">1</div>
            <div class="cart__item__right__count__button">+</div>
          </div>
          <div class="cart__item__right__remove">
            <img
